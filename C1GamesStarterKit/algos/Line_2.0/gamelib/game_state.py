...
class GameState:
    ...
    def __init__(self, config, serialized_string,actionphase):
        ...
        self.opponent_movement_path = None
        self.opponent_movement_heatmap = None
    ...
    def __parse_state(self, state_line):
        ...
        if((len(state["events"]["breach"]) != 0) and self.actionphase):
            a=1#debug_write("Action phase info: {}".format(state_line))
    ...
    def __create_parsed_units(self, units, player_number):
        ...
        for i, unit_types in enumerate(units):
            for uinfo in unit_types:
                ...
                if(self.turn_number == 2 and player_number == 1 and not is_stationary(unit_type)):
