...
class GameState:
    ...
    def find_path_to_edge(self, start_location, target_edge=-1):
        ...
        if target_edge == -1:
            if start_location in self.game_map.get_edge_locations(self.game_map.TOP_RIGHT):
                target_edge = self.game_map.BOTTOM_LEFT
            elif start_location in self.game_map.get_edge_locations(self.game_map.TOP_LEFT):
                target_edge = self.game_map.BOTTOM_RIGHT
            elif start_location in self.game_map.get_edge_locations(self.game_map.BOTTOM_LEFT):
                target_edge = self.game_map.TOP_RIGHT
            elif start_location in self.game_map.get_edge_locations(self.game_map.BOTTOM_RIGHT):
                target_edge = self.game_map.TOP_LEFT
            else:
                warnings.warn("Attempted to perform pathing without a target edge without starting on an edge at {}".format(start_location))
                return
